...
本文记录本轮接手后至首个跨平台发布版之间的主要变化。当前冻结版本为 **v0.2.6**。
...
- `transport/` 中同时保存已完成的 macOS arm64 包、Windows x64 包和统一源码包。
- 用户提供的 Mac 构建产物与原始 Mac 源码归档保持不变；统一源码包是在此基础上合并 Windows 兼容后的最终交付源码。
