 ...
 *
 * (C) Copyright 2009-2018, by Sandy McGuffog and Contributors.
 *
 ...
 * 09 Apr 2018 : V1.6; Fix for a rare issue where mtrixes are small in one dimension
 * 31 Jul 2018 : V1.7; Support for nonlinear HueSatMaps
 *
 */
...
void serialize_dng_vector(xmlNodePtr parent_node, const dng_std_vector<dng_point_real64> *dvector, const char *name)
{	
...
bool parse_dng_vector(xmlDocPtr doc, xmlNodePtr parent_node, dng_std_vector<dng_point_real64> &dvector, const char *name)
{	
	if (xmlStrEqual(parent_node->name, BAD_CAST name)) {
		bool parseError = false;
		int itemCount = 0;
...
//	xmlCreateIntSubset(doc, BAD_CAST "root", NULL, BAD_CAST "AdobeDCP.dtd");
    ...
    serialize_uint32(root_node, fHueSatMapEncoding, "ProfileHueSatMapEncoding");
			...
			if (xmlStrEqual(cur_node->name, BAD_CAST "dcpData")) {
				for (xmlNode *dcp_node =  cur_node->children; dcp_node; dcp_node = dcp_node->next) {
					if (dcp_node->type == XML_ELEMENT_NODE) {
                        ...
                        else if (parse_uint32(doc, dcp_node, fHueSatMapEncoding, "ProfileHueSatMapEncoding")) {}
...
{	
    ...
    s = satDivisions < 2 ? 0.0f : (real32) sDiv / (real32) (satDivisions - 1);
    v = valDivisions < 2 ? 0.0f : (real32) vDiv / (real32) (valDivisions - 1);
    ...
    sDiv = satDivisions < 2 ? 0 : (int32) (((real32) satDivisions - 1) * s);
    vDiv = valDivisions < 2 ? 0 : (int32) (((real32) valDivisions - 1) * v);
